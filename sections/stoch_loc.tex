...
a logarithmic factor of \(\text{Var}_\mu[\|\cdot\|]\). Thus, as translating the barycenter of \(\mu\)
...
Fix \(\phi : \mathbb{R}^n \to \mathbb{R}\) some bounded continuous function and let \((M_t)\) be the 
...
\begin{align*}
  \text{Var}[M_t] + \mathbb{E}[\text{Var}[M_\infty \mid \mathscr{F}_t]] 
    ...
    & = \mathbb{E}[M_t^2] + \mathbb{E}[\mathbb{E}[M_\infty^2 \mid \mathscr{F}_t]] 
      = \mathbb{E}[M_\infty^2]= \text{Var}[M_\infty],
\end{align*}
where we used the fact that \(\mathbb{E}[M_t] = \bar{\mu} = 0\) for the second equality.
...
\(A_t \to 0\) point-wise as \(t \to \infty\). This is important for us to establish the existence of the limit 
of \((a_t)\) and \((M_t)\). However, as outlined above, we will also require some quantitative bounds for 
\(A_t\). We will in this subsection compute some properties of \(A_t\) useful for this 
