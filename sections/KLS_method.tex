...
\begin{lemma}[\cite{Klartag_2006}, Equation 10]
  There exists a universal constant \(C\) such that for all 
  \(f : \mathbb{R}^n \to \mathbb{R}\) the density of a log-concave isotropic measure, we have 
  \[f(x) \le f(0)e^{- |x| / C},\]
  for all \(x \in \mathbb{R}^n\) with \(\|x\| \ge Cn\).
\end{lemma}
As a result, the region outside of said ball contributes to the exponential concentration by 
at most
\begin{align*}
  \mu(|\phi - \mathbb{E}_\mu[\phi]| \ge t \cap C B_n(0)) 
  & \le \int_{\{|\phi(x)| \ge t\} \cap CB_n(0)} e^{-|x| / c} \dd \\ 
  & \simeq \int_{t \vee n} r^{n - 1} e^{-r} \dd r = O(e^{-t}).
\end{align*}
Namely, the exponential concentration constant of \(\mu\) outside of the ball is universally bounded.
Hence, it follows by Theorem~\ref{thm:milman2} that it is sufficient to bound the concentration of 
\(\mu\) within \(B_n(0)\).
...
Thus, to bound \(\text{Var}[M_t]\), the idea is to choose an appropriate stopping time \(\tau\) to 
...
As this lemma requires the matrix to be diagonal, denoting \(e_1, \ldots, e_n\) as the normalized 
...
Again applying Itô's formula, we have that
\begin{align*}
...
Whilst this deduction only holds when ignoring the stochastic term \(\langle v_t, \dd W_t\rangle\), one can 
...
Namely, the martingale term grows at a rate of at most \(\alpha \Phi^\alpha(t)\sqrt{t}\) which is much slower 
