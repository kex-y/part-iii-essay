...
simply its variance. This is illustrated by the following proposition. 
...
to say suggest something of the form ``all log-concave measures and concentrated by a constant which is 
invariant of the measure''. However, as currently stated, this statement is obviously false as spreading out 
the measure increases the concentration. Furthermore, we need to be careful since, unlike the Gaussian, the 
...
We remark that all isotropic measure \(\mu\) satisfies \(\text{Var}_\mu[T] \le 1\) for all 1-Lipschitz linear maps 
...
With the above property in mind, we observe a trivial bound on the concentration of \(\mu\):
...
in finding the set of unit volume with minimum surface area. In the case of the \(\mathbb{R}^n\) equipped with the 
Lebesgue measure, we have known since the ancient Greeks that the solution is the unit ball. 
With this in mind, the KLS conjecture can be viewed geometrically as a natural extension of the isoperimetric problem to 
general measures on \(\mathbb{R}^n\).
...
satisfying \(\mu(A) = 1\) with minimum boundary measure. 
