...
\begin{proposition}\label{prop:linear-tilt_ACV}
  ...
  \[\dd \text{Var}[M_t] \le \text{Var}[M_t] \mathbb{E}[\|A_t\|_\text{op}] \dd t,\]
...
\(\text{Var}[M_t] = \mathbb{E}[M]_t\). As a result of this, we will now analyze \(\dd[] [M]_t\).
...
\begin{proof}
  With the above remarks in mind, by taking expectation, it is sufficient to bound \(\dd[] [M]_t\) 
  ...
  \begin{equation}\label{eq:red_a}
    ...
    \end{split}
  \end{equation}
  where again, the inequality follows by the Cauchy-Schwarz inequality. Thus, by taking expectation on both sides of the inequality, 
