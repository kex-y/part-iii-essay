...
Then, again using equation~\ref{eq:stoch_loc_alt}, we note that any realizations of \((F_t(x))\) is eventually 
decreasing in \(t\) if \(x \neq 0\). Hence, \(\sup_t \int \|\nabla \phi_{ij}\|^2 \dd \mu_t = 
\sup_t \int (x_i^2 + x_j^2) \mu_t(\dd x) < \infty\). Thus, by 
...
\begin{proof}
  ...
  \[a_t = \int x \mu_t(\dd x) \xrightarrow{a.e.} \int x \delta_{a_\infty}(\dd x) =: a_\infty \text{ as } t \to \infty\]
...
Using the same proof as corollary~\ref{cor:lim_dis}, we observe \(M_t\) converges a.e. to some \(M_\infty\) 
with law \(\phi_* \mu\).
