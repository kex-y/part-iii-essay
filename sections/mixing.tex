...
bounding the spectral gap and consequently the mixing time of the Glauber dynamics.
...
\(\mu\). This procedure results in a Markov chain which turns out to have the Ising model as its stationary distribution. 
...
\begin{proof}
  ...
  = \inf_{\phi : \mathcal{X} \to \mathbb{R}} 
    \frac{1}{2 \text{Var}_\mu[\phi]} \int (\phi(x) - \phi(y))^2 K(x, \dd y) \mu(\dd x),\]
  the result follows by taking infimum on both sides.
\end{proof}
