...
sufficiently such that we reobtain a bound for \([M]_t\) after integrating. In particular, 
we will show \(\dd[] [M]_t\) is bounded by a quantity concerning \(A_t\). This should not be at all 
surprising as both \(\dd[] [M]_t\) and \(A_t\) describes the variation of \(M_t\) at a infinitesimal time 
neighborhood of \(t\).
...
\begin{equation}
  ...
  \end{split}
\end{equation}
where the inequality is due to Cauchy-Schwarz and \(\|\cdot\|_{\text{op}}\) denotes the operator norm. 
...
\(A_t\). We will in this subsection compute some properties of \(A_t\) useful for this purpose.
