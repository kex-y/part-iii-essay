...
\begin{conjecture}[Kannan-Lovász-Simonovitz, \cite{Eldan_notes}]
  Denoting \(\mathscr{M}_{\text{con}}\) the set of all log-concave probability measures \(\mu\) on 
  ...
  \(\mu \in \mathscr{M}_{\text{con}}\), \(\mu\) is \(C\)-concentrated.
\end{conjecture}
...
\begin{conjecture}[Thin-shell, \cite{Eldan_2013}]
  Taking \(\mathscr{M}_{\text{con}}\) as above, there exists a universal constant \(C\) such that 
  for all \(\mu \in \mathscr{M}_{\text{con}}\), we have
...
\begin{theorem}[Eldan, \cite{Eldan_2013}]\label{thm:KLS_to_TS}
  Denoting \(\mathscr{M}_{\text{con}}\) as above, we define 
  \[C_{\text{con}} := \inf \left\{C \mid \text{\(\forall \mu \in \mathscr{M}_{\text{con}},\) 
    \(\mu\) is \(C\)-concentrated}\right\},\]
  and 
  \[C_{\text{TS}} := \inf \left\{C \mid \text{\(\forall \mu \in \mathscr{M}_{\text{con}},\) 
      \(\sqrt{\text{Var}_\mu[\|\cdot\|]} \le C\)}\right\}
      = \sup_{\mu \in \mathscr{M}_{\text{con}}} \sqrt{\text{Var}_\mu[\|\cdot\|]},\]
  we have,
  \[C_{\text{TS}} \le C_{\text{con}} \le C_{\text{TS}} \log n.\]
\end{theorem}
