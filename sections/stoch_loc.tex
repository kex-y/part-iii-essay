...
of \((a_t)\) and \((M_t)\). However, as shown above, we now require some quantitative bounds for 
\(A_t\). For this purpose, we first compute some useful properties of \(A_t\).
...
With this computation in mind, we now proceed to bound the operator norm of \(A_t\). In particular, 
...
\begin{equation}
  \Phi_t^{\alpha} = \sum_{i = 1}^n |\lambda_i(t)|^{\alpha} = \|(\lambda_i(t))_{i = 1}^n\|_\alpha^\alpha
\end{equation} 
for some \(\alpha > 0\). Furthermore, as \(A_t\) is positive 
semi-definite, \(\lambda_i(t) \ge 0\) for all \(i = 1, \cdots, n\) and thus we have
\(\Phi_t^\alpha = \sum_{i = 1}^n \lambda_i(t)^{\alpha}\). Again, to proceed, we will attempt to compute 
\(\dd \Phi_t^\alpha\) at some \(t = t_0 > 0\) utilizing the following lemma.
...
\begin{equation}
  \begin{split}
    \dd a_{ij}(t) & = \left\langle e_i, \left(\int (x - a_t)^{\otimes 2} 
      \langle x - a_t, \dd W_t \rangle \mu_t(\dd x)\right) e_j \right\rangle - \langle e_i, A^2_t e_j\rangle \dd t\\
      & = \left\langle\int \langle e_i, (x - a_t)^{\otimes 2} e_j\rangle
        (x - a_t) \mu_t(\dd x), \dd W_t\right\rangle - \langle e_i, A^2_t e_j\rangle \dd t.
  \end{split}
\end{equation}
Since by construction, \(A_{t_0}\) is diagonal with respect to the eigenbasis \((e_i)_{i = 1}^n\), we have 
\(\langle e_i, A^2_{t_0} e_j\rangle = \langle e_i, A_{t_0} e_j\rangle^2 \ge 0\) and thus the term \(- \langle e_i, A^2_t e_j\rangle \dd t\) 
contributes negatively around \(t_0\). Hence, as we are interested in bounding \(\dd a_{ij}(t)\) from 
above, we may ignore the second term. With this in mind, denoting 
\[\xi_{ij} = \int \langle e_i, (x - a_t)^{\otimes 2} e_j\rangle (x - a_t) \mu_t(\dd x),\] 
we have \(\dd a_{ij}(t) \le \langle \xi_{ij}, \dd W_t\rangle\). Thus, combining this with lemma~\ref{lem:diff_eig}, 
...
\begin{equation}
  \begin{split}
    ...
    & \le \langle \xi_{ii}, \dd W_t\rangle + \sum_{j \neq i} \frac{\dd[][a_{ij}]_t}{\lambda_i - \lambda_j}
      \le \langle \xi_{ii}, \dd W_t\rangle + \sum_{j \neq i} \frac{\|\xi_{ij}\|^2}{\lambda_i - \lambda_{j}}.
  \end{split}
\end{equation}
at \(t = t_0\).